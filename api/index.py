...
class SimpleMemory:
    ...
    def add_user_message(self, content: str):
        self.messages.append(SimpleMessage("user", content))
    def add_ai_message(self, content: str):
        self.messages.append(SimpleMessage("assistant", content))
...
def get_session_memory(session_id: str) -> SimpleMemory:
...
async def root_health():
    return {"status": "online", "message": "ProspectIQ IDBI AI API is live and operational"}
...
async def chat_endpoint(request: ChatRequest):
        ...
        if provider == "groq" and (groq_key or (gemini_key and gemini_key.startswith("gsk_"))):
            ...
            if key_to_use and key_to_use.startswith("gsk_"):
                ...
                for m in recent_messages:
                    formatted_msgs.append({"role": m.type, "content": m.content})
    ...
    try:
        if not full_response:
            raise Exception("No AI provider responded, trigger fallback.")
